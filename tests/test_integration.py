...
class TestAITesterState:
    ...
    def test_state_has_required_fields(self):
        """状态应包含必要字段"""
        required_fields = ["target_file", "target_code", "iteration", "max_iterations"]
...
"""
        ...
        result = agent.execute(test_code=test_code, target_file=os.path.join(temp_test_dir, "calculator.py"))
...
"""
        ...
        result = agent.execute(
            test_code=test_code, target_file=os.path.join(temp_test_dir, "calculator.py"), target_function="test_add"
        )
...
class TestPlannerAgent:
    ...
    def test_planner_plan_with_missing_logic_analysis(self):
        ...
        mock_response = json.dumps({"function_name": "subtract", "test_cases": []})
...
class TestDebuggerAgent:
    ...
    def test_debug_classifies_error(self):
        ...
        # 模拟 SyntaxError 输出
        mock_response = json.dumps(
            {"root_cause": "语法错误", "error_category": "syntax", "fix_strategy": "修正语法", "patch": ""}
        )

        agent = DebuggerAgent()
        with patch.object(agent, "_call_llm", return_value=mock_response):
            result = agent.debug(
                target_code=SAMPLE_TARGET_CODE, test_output="SyntaxError: unexpected EOF", failed_cases=[]
            )
    ...
    def test_debug_with_rag_references(self):
        ...
        rag_refs = [{"original_code": "def add(a, b): return a - b", "patch": "def add(a, b): return a + b"}]
