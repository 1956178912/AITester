...
> Last updated: 2026-09-20 (0.6 P0 fix batch: LLM OpenAI path zero-retry → exponential-backoff failover / venv stats dual-lock separation / ghost-switch implementation (`API_CIRCUIT_BACKOFF` + `API_PROMETHEUS_EXPORT` wired into `api_health` + `api_manager`) / multi_candidate double-patch-apply elimination / 15 ruff warnings cleared; full 1612 test cases / 94% coverage)
