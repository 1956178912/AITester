...
| **最新优化** | ✅ 2026-09-15 全项目收敛轮次（config 集中化 + 死代码清理 + 默认关功能修复：删除 CROSS_FILE/ASSERTION 死常量、SWE_BENCH_ENRICHMENT 收敛 config、修复 executor 标准库清单误列 diskcache、修复跨文件降级写不进盘、RAG 检索器写锁、refine 接线收敛，全量 1270 passed / 覆盖率 92%）；详见 [CHANGELOG](CHANGELOG.md) |
