...
>
> **更新（2026-09-14 批次③）**：评估指标多维化首批（1.1/1.2 分析层增强）已推进当前基线至 1163 passed；本文件 1158 相关条目保留为历史批次记录。
...
>
> **批次③ 注记（2026-09-14）**：本轮在批次②之上推进 1.1/1.2 分析层增强（`analyze_results.py` 新增修复收敛效率与多维质量代理两个可回归章节），当前全量基线已更新为 **1163 passed**；上文 1158 条目保留为历史批次记录。
...
- **4.2 Docker 实际启用**（该轮 4.2 指 Docker 启用，与后续 4.2 半开探测批次编号复用但语义不同，勿混淆）：维持 D-06 预留接口（use_docker=False），需确认实验环境有 Docker daemon；
